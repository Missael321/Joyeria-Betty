...
<h3 class="text-center text-primary my-5"><b>Consultar Proveedores</b></h3>
    <form action="#" method="post" id="frmbuscar">
      <div class="container text-center">
        <div>
          <div class="card d-inline-block">
          <h5 class="card-header">Buscar Proveedor</h5>
          <div class="card-body">
            <div class="input-group">
              <input type="text" class="form-control" placeholder="Nombre del Proveedor" name="nombreProveedor" id="nombreProveedor" autocomplete="off">
        ...
        <?php 
          if(isset($_POST['nombreProveedor']) ){
            ...
            $sql = "SELECT * FROM proveedor WHERE nombreProveedor LIKE '$_POST[nombreProveedor]%' ORDER BY nombreProveedor";
            $result = mysqli_query($link, $sql);
            $contar = mysqli_num_rows($result);
            if ($contar == 0) { ?>
              <h1 class="display-4 text-center">No se encontró al Proveedor</h1>
              <?php
            } else {
         ?>

      <h3 class=" text-center text-primary my-5 mx-5"><b>Provedores Registrados</b></h3>
      <div class="container">
    ...
    <?php 
     ...
     <tbody>
     <tr>
      <?php $ID_proveedor = $fila['ID_proveedor'] ?>
      <td class="text-uppercase"><?php echo $fila['RFC_proveedor'] ?></td>
      <td><?php echo $fila['nombreProveedor'] ?></td>
      <td><?php echo $fila['telefono'] ?></td>
      <td><?php echo $fila['correo'] ?></td>
      <?php echo "
      <td class='active'><a href='modificar.php?ID_proveedor=$ID_proveedor'>
      <img  width='32px' height='32px' src='../images/editar.png'/>
      </a></td>
      <td>
        <a onClick='confirmar_eliminacion($ID_proveedor)'>
  ...
  <script language="javascript"> 
    function confirmar_eliminacion(ID_proveedor){ 
    var aceptar=confirm("\u00bfRealmente deseas eliminar el proveedor?\nAl hacerlo todos los productos que pertenezcan a este proveedor tambien se eliminaran"); 
    if (aceptar) 
    {
